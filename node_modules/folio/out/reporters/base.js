...
exports.stripAscii = exports.formatFailure = exports.BaseReporter = void 0;
...
function formatTestHeader(config, test, indent, index) {
    const tokens = [];
    const spec = test.spec;
    let relativePath = path_1.default.relative(config.testDir, spec.file) || path_1.default.basename(spec.file);
    ...
    tokens.push(safe_1.default.red(pad(header, '=')));
    return tokens.join('\n');
}
...
function formatError(error, file) {
    ...
    if (stack) {
        ...
        tokens.push('');
        tokens.push(safe_1.default.dim(stack.substring(preamble.length + 1)));
    }
    else {
        tokens.push('');
